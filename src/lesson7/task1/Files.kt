...
fun deleteMarked(inputName: String, outputName: String) {
    ...
    File(inputName).bufferedReader().forEachLine {
        if (it.isNotEmpty()) {
            if (it[0] != '_') writer.appendLine(it)
        } else writer.newLine()
    }
    writer.close()
}
 ...
 */
fun maxLength(list: MutableList<String>): Int {
    var max = 0
    for (i in list.indices) {
        var el = list[i].trim()
        if (el.matches(Regex("""\s*"""))) {
            list[i] = ""
            continue
        }
        el = el.replace(Regex("""\s+"""), " ")
        val len = el.length
        if (len > max) max = len
        list[i] = el
    }
...
fun alignFileByWidth(inputName: String, outputName: String) {
    val reader = File(inputName).bufferedReader().readLines().toMutableList()
    val writer = File(outputName).bufferedWriter()
    val max = maxLength(reader)
    if (max == 0) return writer.close()
    reader.forEach {
        val words = it.split(" ")
        writer.write(words[0])
        if (words.size > 1) {
            val len = it.length - words.size + 1
            val a = max - len
            val b = words.size - 1
            val spacesRight = a / b
            val extraSpaces = a % b
            for (i in 1..extraSpaces)
                writer.write(" ".repeat(spacesRight + 1) + words[i])
            for (i in extraSpaces + 1 until words.size)
                writer.write(" ".repeat(spacesRight) + words[i])
        }
        writer.newLine()
    }
    writer.close()
